...
const OfferStats = () => {
  ...
  const calculateStats = (offers: IOfferCard[]) => {
    ...
    offers.forEach((offer) => {
      ...
      offersByLocation[offer.location] =
        (offersByLocation[offer.location] || 0) + 1;
      ...
      const daysAgo =
        (Date.now() - publicationDate.getTime()) / (1000 * 3600 * 24);
      if (daysAgo <= 30) {
        offersByTime["Últimos 30 días"] =
          (offersByTime["Últimos 30 días"] || 0) + 1;
      } else {
        offersByTime["Hace más de 30 días"] =
          (offersByTime["Hace más de 30 días"] || 0) + 1;
      }
    ...
    setStats({
      ...
      offersByTime,
      mostActiveProfile:
        mostActiveProfile === "RECRUITER" ? "Reclutador" : "Agencia",
    });
  };
  ...
  if (loading) {
    return (
      <div className="text-center text-gray-600 mt-28">
        ...
        {activeTab === "perfiles" && (
          <div className="bg-white border p-4 rounded shadow-md">
            ...
            <p className="text-gray-800">
              Perfil más activo: {stats.mostActiveProfile}
            </p>
          </div>
        )}
      </div>
